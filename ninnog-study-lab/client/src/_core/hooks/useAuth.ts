...
export function useAuth(options?: UseAuthOptions) {
  ...
  return useMemo(() => ({
    ...
    logout,
    redirectOnUnauthenticated: options?.redirectOnUnauthenticated ?? false,
    redirectPath: options?.redirectPath,
  }), [meQuery.data, meQuery.error, meQuery.isLoading, logoutMutation.error, logoutMutation.isPending, logout, options?.redirectOnUnauthenticated, options?.redirectPath]);
}
